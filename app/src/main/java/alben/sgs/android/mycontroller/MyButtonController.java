...
public class MyButtonController implements OnTouchListener {

	public GameApp gameApp;

	public MyButtonController(GameApp app) {
		this.gameApp = app;
	}
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case R.id.ChuPai: {
			if (this.gameApp.gameLogicData.selectWJAfterChuPai)
				this.handleSelectWuJiangEvent(v, event);
			else
				this.handleChuPaiBtnEvent(v, event);
			break;
		}
		case R.id.FangQi: {
			this.handleFangQiBtnEvent(v, event);
			break;
		}
		case R.id.QiPai: {
			this.handleQiPaiBtnEvent(v, event);
			break;
		}
		case R.id.TuoGuan: {
			this.handleTuoGuanBtnEvent(v, event);
			break;
		}
		case R.id.JiNeng1:
		case R.id.JiNeng2:
		case R.id.JiNeng3: {
			this.handleWuJiangJiNengBtnEvent(v, event);
			break;
		}
		default:
			break;
	...
	public void handleChuPaiBtnEvent(View v, MotionEvent event) {
	...
	public void handleFangQiBtnEvent(View v, MotionEvent event) {
		switch (event.getAction()) {
		case MotionEvent.ACTION_DOWN: {
			v.setBackgroundDrawable(this.gameApp.getResources().getDrawable(
					R.drawable.btn_bg1_down));
	...
	public void handleQiPaiBtnEvent(View v, MotionEvent event) {
		switch (event.getAction()) {
		case MotionEvent.ACTION_DOWN: {
			v.setBackgroundDrawable(this.gameApp.getResources().getDrawable(
					R.drawable.btn_bg1_down));
	...
	public void handleTuoGuanBtnEvent(View v, MotionEvent event) {
		switch (event.getAction()) {
		case MotionEvent.ACTION_DOWN: {
			v.setBackgroundDrawable(this.gameApp.getResources().getDrawable(
					R.drawable.btn_bg1_down));
		...
		case MotionEvent.ACTION_UP: {
			...
			if (this.gameApp.gameLogicData.myWuJiang.tuoGuan) {
				...
				gameApp.libGameViewData.logInfo("你退出托管模式",
						Type.logDelay.NoDelay);
			} else {
				this.gameApp.gameLogicData.myWuJiang.tuoGuan = true;
				gameApp.libGameViewData.logInfo("你进入托管模式",
						Type.logDelay.NoDelay);
			}
			break;
		}
	...
	public void handleSelectWuJiangEvent(View v, MotionEvent event) {
		switch (event.getAction()) {
		case MotionEvent.ACTION_DOWN: {
			v.setBackgroundDrawable(this.gameApp.getResources().getDrawable(
					R.drawable.btn_bg1_down));
