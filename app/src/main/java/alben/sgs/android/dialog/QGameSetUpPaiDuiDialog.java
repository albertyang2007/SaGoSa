...
public class QGameSetUpPaiDuiDialog extends Dialog {
	...
	public void onCreate() {
		setContentView(R.layout.qgame_setup_pdd);

		this.selectedWJ = this.gameApp.wjDetailsViewData.selectedWJ;

		((ImageView) findViewById(R.id.qgame_paidui_top))
		...
		localButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if ((selectedCP != null)
						&& (gameApp.settingsViewData.paiDuiTopCPs.size() < cardPais.length)) {
					gameApp.settingsViewData.paiDuiTopCPs.add(selectedCP);
		...
		localButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if ((inWhichTab == R.id.qgame_paidui_top)
						&& (selectedCP != null)) {
					gameApp.settingsViewData.paiDuiTopCPs.remove(selectedCP);
					updatePaiDuiTopToView();
					resetCardPaiBackGround();
				}
			}
	...
	public void updateHuaShiCardPaiToView(Type.CardPaiClass paramCardPaiClass) {
		...
		for (int j = 0; j < this.cardPais.length; j++)
			this.cardPais[j] = null;
		int viewIndex = 0;
		int m = 0;
	...
	public void updatePaiDuiTopToView() {
		int viewIndex;
		for (viewIndex = 0; (viewIndex < this.gameApp.settingsViewData.paiDuiTopCPs
				.size()) && (viewIndex < this.cardPaiImgView.length); viewIndex++) {
			this.cardPaiImgView[viewIndex].setImageDrawable(this.gameApp
					.getResources().getDrawable(
							(this.gameApp.settingsViewData.paiDuiTopCPs
									.get(viewIndex)).imageNumber));
			this.cardPaiImgView[viewIndex].setEnabled(true);
		}
		// set others to back
		while (viewIndex < this.cardPaiImgView.length) {
	...
	private class CardPaiImageListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			...
			else if (j < gameApp.settingsViewData.paiDuiTopCPs.size())
				selectedCP = ((CardPai) gameApp.settingsViewData.paiDuiTopCPs
						.get(j));
			//
			for (int i = 0; i < cardPaiImgView.length; i++)
				if (i != j)
					...
					QGameSetUpPaiDuiDialog.this.cardPaiImgView[i]
